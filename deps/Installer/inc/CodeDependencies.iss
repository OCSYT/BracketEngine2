...
function ExecuteFile(const Filename, Parameters: string; var ResultCode: Integer): Boolean;
begin
  Result := False;
  ResultCode := 0;
  if ExtractFileExt(Filename) = '.msi' then begin
    Log('/i "' + Filename + '" ' + Parameters);
    Result := ShellExec('', 'msiexec.exe', '/i "' + Filename + '" ' + Parameters, '', SW_SHOWNORMAL, ewWaitUntilTerminated, ResultCode);
  end else begin
...
#else
          if ExecuteFile(ExpandConstant('{tmp}{\}') + Dependency_List[DependencyIndex].Filename, Dependency_List[DependencyIndex].Parameters, ResultCode) then begin
#endif
...
procedure Dependency_AddDotNet90SDK;
var
  RegPath: String;
...
begin
  ...
  begin
    InstallParams := '/lcid ' + IntToStr(GetUILanguage) + ' /passive /norestart';
    Dependency_Add(
      'dotnet-sdk-9.0.101' + Dependency_ArchSuffix + '.exe',
      InstallParams,
      '.NET SDK 9.0.101' + Dependency_ArchTitle,
      Dependency_String(
        'https://download.visualstudio.microsoft.com/download/pr/838bc4f1-96ee-43f9-8e47-2dc2656590c0/15cbe313b18ccbeffdb61cff66f5ef26/dotnet-sdk-9.0.101-win-x86.exe',
